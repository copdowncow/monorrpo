// API URL is empty string because Express serves both frontend and API on same port
const API_URL = '';
...
export const api = {
  ...
  calculatePrice: (balls: number) => apiCall(`/api/bookings/calculate?balls=${balls}`),
  getBookingStatus: (number: string) => apiCall(`/api/bookings/status/${number}`),
  getSlots: (date: string) => apiCall(`/api/slots?date=${date}`),
  getPricing: () => apiCall('/api/pricing'),

  // Admin
  login: (login: string, password: string) =>
    apiCall('/api/auth/login', { method: 'POST', body: JSON.stringify({ login, password }) }),
  ...
  getAllPricing: () => apiCall('/api/pricing/all'),
  createPricing: (data: unknown) => apiCall('/api/pricing', { method: 'POST', body: JSON.stringify(data) }),
  updatePricing: (id: string, data: unknown) =>
    apiCall(`/api/pricing/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
  deletePricing: (id: string) => apiCall(`/api/pricing/${id}`, { method: 'DELETE' }),
  getSettings: () => apiCall('/api/pricing/settings'),
  updateSetting: (key: string, value: string) =>
    apiCall(`/api/pricing/settings/${key}`, { method: 'PATCH', body: JSON.stringify({ value }) }),
};
